...
class UserServiceImpl(
    ...
    override fun signUp(request: UserSignUpRequest): UserSignInDTO {
        val key = "email:uuid:${request.email}"
        ...
        val profile = UserProfile(user.id!!)
        insert(profile)
...
//            select(UserRole::roleId, UserRole::userId).`as`("t1")
        ...
        val userProfile = filterOne<UserProfile> { UserProfile::id eq userId } ?: throw CustomException(UserCustomExceptionCode.USER_PROFILE_NOT_EXISTS)
    ...
    override fun sendCode(request: SendCodeRequest) {
        ...
        if (user != null) {
            throw CustomException(UserCustomExceptionCode.EMAIL_REGISTERED)
        }

        val code = generateVerificationCode()
        val key = "email:code:${request.email}"
        // TODO Body - Replace this body with the content generated by the file generation service
    ...
    override fun validateCode(request: ValidateCodeRequest): String {
        ...
        redisTemplate.opsForValue().set(key2, uuid, 30, TimeUnit.MINUTES)
